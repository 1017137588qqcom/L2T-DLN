...
def to_var(x, requires_grad=True, device='cuda:1'):
...
class MetaLinear(MetaModule):
    def __init__(self, input_size, output_size, bias):
        super().__init__()
        ignore = nn.Linear(input_size, output_size, bias)

        self.register_buffer('weight', to_var(nn.init.kaiming_normal_(ignore.weight.data, mode='fan_out', nonlinearity='relu'), requires_grad=True))
...
class MetaSequential(MetaModule):
    ...
    def __dir__(self):
        keys = super(Sequentialc, self).__dir__()
...
class METALeNet(MetaModule):
    def __init__(self, n_out):
        ...
        layers = []
        layers.append(MetaLinear(16*4*4, 64, bias=True))
        layers.append(nn.ReLU(inplace=True))
        layers.append(MetaLinear(64, 16, bias=True))
        layers.append(nn.ReLU(inplace=True))
        layers.append(MetaLinear(16, n_out, bias=True))
...
class LeNet_Cifar10(MetaModule):
    def __init__(self, num_classes=10):
        super(LeNet_Cifar10, self).__init__()
        layers = []
        layers.append(MetaConv2d(3, 6, kernel_size=3, padding=1))
        # layers.append(MetaBatchNorm2d(6))
        layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
        layers.append(nn.ReLU(inplace=True))

        layers.append(MetaConv2d(6, 16, kernel_size=3, padding=1))
        # layers.append(MetaBatchNorm2d(16))
        layers.append(nn.MaxPool2d(kernel_size=2, stride=2))
        layers.append(nn.ReLU(inplace=True))
        ...
        layers = []
        layers.append(MetaLinear(16 * 8 * 8, 64, bias=True))
        layers.append(nn.ReLU(inplace=True))
        layers.append(MetaLinear(64, 16, bias=True))
        layers.append(nn.ReLU(inplace=True))
        layers.append(MetaLinear(16, num_classes, bias=True))
    ...
    def forward(self, x):
        x = self.main(x)
        x = x.view(-1, 16 * 8 * 8)
        return x, self.fc_layers(x).squeeze()

class LeNet(nn.Module):
    def __init__(self, num_classes=10):
        super(LeNet, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=20, kernel_size=5, bias=True), #20
            nn.BatchNorm2d(20),
            nn.MaxPool2d(2, 2),
            nn.ReLU(),
            nn.Conv2d(20, 50, kernel_size=5, bias=True), #50
            nn.BatchNorm2d(50),
            nn.MaxPool2d(2, 2),
            nn.ReLU()
        )
        self.linear = nn.Sequential(
            nn.Linear(50*4*4, 800, bias=True),
            nn.ReLU(),
            nn.Linear(800, 500, bias=True),
            nn.ReLU(),
            nn.Linear(500, num_classes, bias=True)
        )
...
class LeNet_center(nn.Module):
    def __init__(self, num_classes=10):
        super(LeNet_center, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=20, kernel_size=5, bias=True),  # 20
            nn.MaxPool2d(2, 2),
            nn.ReLU(),
        ...
        self.output = nn.Linear(500, num_classes, bias=True)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_feature=1024):
        ...
        self.linear1 = nn.Linear(in_feature, num_classes) #512*exp*block.expansion
...
def ResNet20(num_classes):
    return ResNet(BasicBlock, [3,3,3], num_classes=num_classes) #2048
