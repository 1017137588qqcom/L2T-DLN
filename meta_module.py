...
class MetaLinear(MetaModule):
    def __init__(self, input_size, output_size, bias, dvs):
        super().__init__()
        ignore = nn.Linear(input_size, output_size, bias)

        self.register_buffer('weight',
                             to_var(nn.init.kaiming_normal_(ignore.weight.data, mode='fan_out', nonlinearity='leaky_relu'),
                                    requires_grad=True, device=dvs))
        if ignore.bias is not None:
            self.register_buffer('bias', to_var(ignore.bias.data, requires_grad=True, device=dvs))
        else:
